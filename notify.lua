...
local NotificationColors = {
	...
	default = Color3.fromRGB(189, 195, 199)
}
...
function prompt(title, text, closeTime, close, notificationType)
	local Prompt = Notification:Clone()
	local Sound = Instance.new('Sound', Notification)

	local accentColor = NotificationColors[notificationType or "default"]
	
	Prompt.Visible = true
	Prompt.Container.Top.Title.Text = title
	Prompt.Container.Body.Content.Text = text
	Prompt.Container.Top.Accent.BackgroundColor3 = accentColor
	Prompt.Container.Top.Accent.BackgroundTransparency = 0.300
...
function lib.prompt(title, description, closeTime, notificationType)
	prompt(title, description, closeTime, true, notificationType)
end

function lib.success(title, description, closeTime)
	prompt(title, description, closeTime or 3, true, "success")
end

function lib.error(title, description, closeTime)
	prompt(title, description, closeTime or 5, true, "error")
end

function lib.warning(title, description, closeTime)
	prompt(title, description, closeTime or 4, true, "warning")
end

function lib.info(title, description, closeTime)
	prompt(title, description, closeTime or 3, true, "info")
end
